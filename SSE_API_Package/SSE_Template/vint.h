...
#define mask_mov(m, v1, v2) _mm512_mask_mov_epi32(vint(v2).val, m, vint(v1).val)
...
public:
    ...
    inline const vint sqrt(){
        ...
        //Compute sqrt
        tmp = _mm_mul_ps(tmp, _mm512_rsqrt28_ps(tmp));
    ...
    vint& max(const vint &b) {
        val = _mm512_max_epi32(val, b.val);
        return *this;
    }

    vint& min(const vint &b) {
        val = _mm512_min_epi32(val, b.val);
        return *this;
    }
...
inline const vint max(vint a, const vint &b) {
    a = a.max(b.val); 
    return a;

}

inline const vint min(vint a, const vint &b) {
    a = a.min(b.val); 
    return a;
