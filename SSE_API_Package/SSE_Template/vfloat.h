...
public:
    ...
    inline const vfloat sqrt(){
        vfloat tmp = _mm_mul_ps(val, _mm512_rsqrt28_ps(val));
        return tmp;
    }

    vfloat &max(const vfloat &b) {
        val = _mm512_max_ps(val, b.val);
        return *this;
    }

    vfloat &min(const vfloat &b) {
        val = _mm512_min_ps(val, b.val);
        return *this;
    }
...
inline const vfloat max(vfloat a, const vfloat &b) {
    a = a.max(b.val);
    return a;
} 

inline const vfloat min(vfloat a, const vfloat &b) {
    a = a.min(b.val);
    return a;
} 
