...
public:
    ...
    static void add_mask(const mask &_mask, const __SSE &_old);
    static void set_nomask(const __SSEi &_oldi);
...
void Mask::add_mask(const mask &_mask, const __SSE &_old)
{
    m &= _mask;
    old = _old;
}

void Mask::set_nomask(const __SSEi &_oldi)
{
    oldi = _oldi;
}
