  ...
  while(wake_size != 0) {
    ...
    gettimeofday(&tv1, &tz1);
      ...
      _mm512_prefetch_i32gather_ps(vny_next, width, 4, _MM_HINT_T0);
		  ...
		  __m512 vdx = _mm512_mask_i32gather_ps(vnoneps, mactive, vnx, width, 4);
		  __m512 vdy = _mm512_mask_i32gather_ps(vnoneps, mactive, vny, width, 4);


		  __m512 vnewdy = _mm512_mask_min_ps(vnoneps, mactive, vdx, vw);

		  __mmask16 mupdate = _mm512_mask_cmp_ps_mask(mactive, vdy, vnewdy, _CMP_LT_OS);
		  _mm512_mask_i32scatter_ps(width_new, mupdate, vny, vnewdy, 4);
