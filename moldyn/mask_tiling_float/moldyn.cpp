...
__m512i vzeroi = _mm512_set1_epi32(0);
__m512 vone = _mm512_set1_ps(1);
__m512i vonei = _mm512_set1_epi32(1);
...
void tiling() {
  ...
  for(int i=0;i<ninter;i++) {
    ...
    tiles[tx*dtiles+ty].push_back(make_pair(inter1[i], inter2[i]));
  }
...
int main(int argc, char **argv)
{
  ...
  for (tstep = 0; tstep < n_tstep; tstep++) {
    ...
    __m512i vi, vj;
    __m512 vxi, vxj, vyi, vyj, vzi, vzj;
    __m512i vbound = _mm512_set1_epi32(n_inter);
    ...
    while(ii < n_inter) {
      ...
      vindex = _mm512_add_epi32(vindex, _mm512_set1_epi32(ii));
      ii += nbits[mload];

      __mmask16 mbound = _mm512_mask_cmplt_epi32_mask(mload, vindex, vbound);
      mload &= mbound;

      vi = _mm512_mask_i32gather_epi32(vi, mload, vindex, inter1, 4);
      vj = _mm512_mask_i32gather_epi32(vj, mload, vindex, inter2, 4);

      vxi = _mm512_mask_i32gather_ps(vxi, mload, vi, v1, 4);
      vxj = _mm512_mask_i32gather_ps(vxj, mload, vj, v1, 4);
      vyi = _mm512_mask_i32gather_ps(vyi, mload, vi, v2, 4);
      vyj = _mm512_mask_i32gather_ps(vyj, mload, vj, v2, 4);
      vzi = _mm512_mask_i32gather_ps(vzi, mload, vi, v3, 4);
      vzj = _mm512_mask_i32gather_ps(vzj, mload, vj, v3, 4);
      ...
      __mmask16 mcutoff = _mm512_cmp_ps_mask(vrd, vcutoffsqure, _CMP_LT_OS);
      if(mcutoff) {
        ...
        __m512i vconfi = _mm512_mask_conflict_epi32(vonei, mcutoff, vi);
        __mmask16 mloadi = _mm512_cmpeq_epi32_mask(vconfi, vzeroi);
        __m512i vconfj = _mm512_mask_conflict_epi32(vonei, mcutoff, vj);
        __mmask16 mloadj = _mm512_cmpeq_epi32_mask(vconfj, vzeroi);
        mload = mloadi & mloadj;

        __m512 vfxi = _mm512_mask_i32gather_ps(vfx, mload, vi, f1, 4);
        __m512 vfyi = _mm512_mask_i32gather_ps(vfy, mload, vi, f2, 4);
        __m512 vfzi = _mm512_mask_i32gather_ps(vfz, mload, vi, f3, 4);

        vfxi = _mm512_add_ps(vfxi, vfx);
        vfyi = _mm512_add_ps(vfyi, vfy);
        vfzi = _mm512_add_ps(vfzi, vfz);

        _mm512_mask_i32scatter_ps(f1, mload, vi, vfxi, 4);
        _mm512_mask_i32scatter_ps(f2, mload, vi, vfyi, 4);
        _mm512_mask_i32scatter_ps(f3, mload, vi, vfzi, 4);

        __m512 vfxj = _mm512_mask_i32gather_ps(vfx, mload, vj, f1, 4);
        __m512 vfyj = _mm512_mask_i32gather_ps(vfy, mload, vj, f2, 4);
        __m512 vfzj = _mm512_mask_i32gather_ps(vfz, mload, vj, f3, 4);

        vfxj = _mm512_sub_ps(vfxj, vfx);
        vfyj = _mm512_sub_ps(vfyj, vfy);
        vfzj = _mm512_sub_ps(vfzj, vfz);

        _mm512_mask_i32scatter_ps(f1, mload, vj, vfxj, 4);
        _mm512_mask_i32scatter_ps(f2, mload, vj, vfyj, 4);
        _mm512_mask_i32scatter_ps(f3, mload, vj, vfzj, 4);
      }
      mload |= ~mcutoff;
      active_nbits += nbits[mload];
      total_nbits += 16;
    }
    double after = get_timer();
    cout << "iteration: " << tstep << endl;
    time += after - before;
    cout << "simd utilization: " << 1.0 * active_nbits / total_nbits << endl;
