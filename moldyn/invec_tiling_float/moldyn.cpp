...
long overhead = 0;
inline void invec_reduce(__mmask16 mconf, __m512i vny, __m512 vdy1, __m512 vdy2, __m512 vdy3, __m512 &vnewdy1, __m512 &vnewdy2, __m512 &vnewdy3, __mmask16 mupdate) {
  ...
  if(mconft != 0xFFFF) { 
    ...
    if(!(mconft & 0x2)) {
      ...
      vdy1 = _mm512_mask_mov_ps(vdy1, mt & (~mt+1), _mm512_set1_ps(_mm512_mask_reduce_min_ps(mt, vdy1)));
      vdy2 = _mm512_mask_mov_ps(vdy2, mt & (~mt+1), _mm512_set1_ps(_mm512_mask_reduce_min_ps(mt, vdy2)));
      vdy3 = _mm512_mask_mov_ps(vdy3, mt & (~mt+1), _mm512_set1_ps(_mm512_mask_reduce_min_ps(mt, vdy3)));
      mconft |= mt;
    }
    if(!(mconft & 0x4)) {
      overhead++;
...
int main(int argc, char **argv)
{
  ...
  for (tstep = 0; tstep < n_tstep; tstep++) {
    ...
    for(ii=0; ii<n_inter; ii+=16) {
      ...
      __m512 vxi = _mm512_i32gather_ps(vi, v1, 4);
      __m512 vxj = _mm512_i32gather_ps(vj, v1, 4);
      __m512 vyi = _mm512_i32gather_ps(vi, v2, 4);
      __m512 vyj = _mm512_i32gather_ps(vj, v2, 4);
      __m512 vzi = _mm512_i32gather_ps(vi, v3, 4);
      __m512 vzj = _mm512_i32gather_ps(vj, v3, 4);
      ...
      if(mcutoff) {
        ...
        __m512i vconf = _mm512_maskz_conflict_epi32(mcutoff, vi);
        __mmask16 mconf = _mm512_cmpeq_epi32_mask(vconf, vzeroi);
        invec_reduce(mconf, vi, vfxi, vfyi, vfzi, vfxi, vfyi, vfzi, mcutoff);

        vfxi = _mm512_add_ps(vfxi, vfx);
        vfyi = _mm512_add_ps(vfyi, vfy);
        vfzi = _mm512_add_ps(vfzi, vfz);

        _mm512_mask_i32scatter_ps(f1, mconf&mcutoff, vi, vfxi, 4);
        _mm512_mask_i32scatter_ps(f2, mconf&mcutoff, vi, vfyi, 4);
        _mm512_mask_i32scatter_ps(f3, mconf&mcutoff, vi, vfzi, 4);
        ...
        vconf = _mm512_maskz_conflict_epi32(mcutoff, vj);
        mconf = _mm512_cmpeq_epi32_mask(vconf, vzeroi);
        invec_reduce(mconf, vj, vfxj, vfyj, vfzj, vfxj, vfyj, vfzj, mcutoff);
        ...
        _mm512_mask_i32scatter_ps(f1, mconf&mcutoff, vj, vfxj, 4);
        _mm512_mask_i32scatter_ps(f2, mconf&mcutoff, vj, vfyj, 4);
        _mm512_mask_i32scatter_ps(f3, mconf&mcutoff, vj, vfzj, 4);
      }
